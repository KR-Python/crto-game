...
## Connect to game_loop.tick_completed.
...
func _create_factory_row(factory: Dictionary) -> Control:
	...
	name_label.text = factory.get("name", "Factory")
	...
	if current_unit != "":
		...
		bar.value = factory.get("progress", 0.0) * 100.0
		top_row.add_child(bar)
		var eta_label := Label.new()
		eta_label.text = " ETA:%ds" % factory.get("eta_ticks", 0)
	...
	for i in range(MAX_QUEUE_SLOTS):
		...
		if i < queue.size():
			slot_btn.text = _unit_abbrev(queue[i])
			slot_btn.tooltip_text = queue[i]
			slot_btn.pressed.connect(_on_queue_slot_pressed.bind(factory_id, i))
		else:
			slot_btn.text = "—"
...
func _unit_abbrev(unit_type: String) -> String:
	...
	return parts[-1].substr(0, 3).to_upper() if parts.size() > 0 else unit_type.substr(0, 3).to_upper()

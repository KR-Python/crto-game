...
## Shared team communication log. Max 20 entries. Color-coded by role.
...
func add_message(role: String, message: String, request_id: int = -1) -> void:
	_append_entry({"role": role, "message": message, "request_id": request_id, "is_request": false})

func add_request(from_role: String, to_role: String, request: Dictionary, request_id: int) -> void:
	var summary := "[-> %s] %s: %s" % [to_role.to_upper(), request.get("type", "request"), request.get("detail", "")]
	_append_entry({"role": from_role, "message": summary, "request_id": request_id,
		"is_request": true, "to_role": to_role, "request": request})
...
func _create_entry_node(entry: Dictionary) -> Control:
	...
	msg.text = "[color=#%s][b]%s:[/b][/color] %s" % [color.to_html(false), role_str.to_upper(), entry.get("message", "")]

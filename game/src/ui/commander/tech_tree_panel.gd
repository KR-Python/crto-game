...
func tick_update(ticks_remaining: int, _ticks_total: int) -> void:
...
func _rebuild_list() -> void:
	...
	for tier in keys:
		...
		tier_header.text = "── Tier %d ──" % tier
		tier_header.add_theme_font_size_override("font_size", 11)
		tier_header.modulate = Color(0.7, 0.9, 1.0)
		_list_container.add_child(tier_header)
		for tech in by_tier[tier]:
			_list_container.add_child(_create_tech_row(tech))


func _create_tech_row(tech: Dictionary) -> Control:
	...
	var cost_ore: int = tech.get("cost_ore", 0)
	var cost_gems: int = tech.get("cost_gems", 0)
	var ticks: int = tech.get("research_ticks", 0)
	var effect: String = tech.get("effect_description", "")
	btn.text = "%s%s\n%d ore  %d gems  |  %ds\n%s" % [
		tech.get("name", tech.id), state_tag, cost_ore, cost_gems, ticks, effect]
